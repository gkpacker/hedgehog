defmodule BinanceMock do
  ...
  defp order_limit(symbol, quantity, price, side) do
    {quantity, _} = quantity |> to_string |> Float.parse()
    {price, _} = price |> to_string |> Float.parse()
  ...
  def handle_info(
    ...
    remaining_sell_orders = Enum.drop(order_book.sell_side, length(filled_sell_orders))
